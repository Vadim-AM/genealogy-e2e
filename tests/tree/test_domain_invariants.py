...
def test_patch_person_death_before_birth_is_422(
...
def test_patch_parent_birth_after_child_is_422(
    ...
    with step("подготовка: создание ребёнка (1985) и родителя (1960) со связью"):
        ...
        relationship_api.create_relationship(
            api,
            rel_type="parent",
            person1_id="dom004-parent",
            person2_id="dom004-child",
        )
...
def test_patch_person_garbage_birth_is_422(
    owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
...
def test_third_parent_relationship_is_rejected(
    signup_via_api: Callable[..., AuthUser], tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("подготовка: создание ребёнка и трёх родителей, привязка двух"):
        ...
        relationship_api.create_relationship(
            api,
            rel_type="parent",
            person1_id="dom002-p1",
            person2_id="dom002-child",
        )
        relationship_api.create_relationship(
            api,
            rel_type="parent",
            person1_id="dom002-p2",
            person2_id="dom002-child",
        )
...
def test_subject_cannot_be_demoted_to_demo_branch(
    owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
...
def test_delete_non_root_person_with_relationship_does_not_500(
    ...
    with step("подготовка: создание ребёнка, родителя и связи"):
        ...
        relationship_api.create_relationship(
            api,
            rel_type="parent",
            person1_id="cascade-parent",
            person2_id="cascade-child",
        )
...
def test_patch_person_huge_notes_is_rejected(
    owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
...
def test_patch_person_huge_surname_is_rejected(
    owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
