...
@pytest.mark.xfail(
    reason="BUG-A11Y-001: после client-side validation-fail на signup "
           "(пустой/битый email) поле не получает aria-invalid=\"true\" "
           "и нет aria-describedby на error-msg. Screen reader не получает "
           "signal. Fix: в submit-handler signup.js, при validation error, "
           "выставить input.setAttribute('aria-invalid', 'true') + "
           "input.setAttribute('aria-describedby', 'email-err').",
    strict=False,
)
def test_signup_invalid_email_sets_aria_invalid(page: Page):
    """A-SU-3: invalid email → input has `aria-invalid="true"`."""
    page.goto("/signup")
    page.wait_for_load_state("domcontentloaded")
    page.locator("#email").fill("not-an-email")
    page.locator("#password").fill("test_password_8plus")
    page.locator("#full_name").fill("Test")
