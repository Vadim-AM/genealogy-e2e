...
from tests._models.platform import (
    AuditLogResponse,
    FeaturesResponse,
...
def get_features(base_url: str) -> FeaturesResponse:
    ...
    r = _httpx.get(f"{base_url}{API.CONFIG_FEATURES}", timeout=TIMEOUTS.api_request)
...
def get_audit_log(api: httpx.Client, **params: Any) -> AuditLogResponse:
    ...
    r = api.get(API.PLATFORM_AUDIT_LOG, params=params)
...
def post_telemetry(api: httpx.Client, batch: list[dict]) -> TelemetryResponse:
    ...
    r = api.post(API.TELEMETRY_EVENTS, json={"batch": batch})
