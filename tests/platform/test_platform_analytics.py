...
def test_device_mix_403_for_non_super(owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]) -> None:
...
def test_device_mix_returns_canonical_shape(
    ...
    with step("действие: запрашиваем device-mix за 30 дней"):
        ...
        data = expect_response(r, label="device-mix shape").status_ok().data
...
def test_device_mix_clamps_days_lower_bound(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
...
def test_device_mix_clamps_days_upper_bound(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
...
def test_device_mix_does_not_leak_pii(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
...
def test_activity_heatmap_403_for_non_super(
...
def test_activity_heatmap_returns_7x24_matrix(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем тепловую карту"):
        ...
        data = expect_response(r, label="heatmap 7x24").status_ok().data
        matrix = data["matrix"]
...
def test_activity_heatmap_returns_canonical_fields(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    """TC-PA-ANALYTICS-2.3: matrix, by_hour, by_weekday, top_hours, top_weekdays, coverage, tz_mode."""
    with step("действие: запрашиваем тепловую карту"):
        r = tenant_client(superadmin_user).get(routes.PLATFORM_ACTIVITY_HEATMAP)
        data = expect_response(r, label="heatmap fields").status_ok().data
...
def test_activity_heatmap_invalid_tz_mode_falls_back_to_utc(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    """TC-PA-ANALYTICS-2.4: tz_mode=garbage → utc (документированный fallback)."""
    r = tenant_client(superadmin_user).get(
        routes.PLATFORM_ACTIVITY_HEATMAP,
        params={"tz_mode": "garbage"},
    )
...
def test_activity_heatmap_user_local_mode_accepted(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем тепловую карту в режиме user_local"):
        r = tenant_client(superadmin_user).get(
            routes.PLATFORM_ACTIVITY_HEATMAP,
            params={"tz_mode": "user_local"},
        )
        data = expect_response(r, label="heatmap user_local").status_ok().data
...
def test_online_now_403_for_non_super(owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]) -> None:
...
def test_online_now_returns_canonical_shape(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем online-now"):
        ...
        data = expect_response(r, label="online-now shape").status_ok().data
...
def test_session_stats_403_for_non_super(
    owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
...
def test_session_stats_returns_canonical_shape(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем session-stats"):
        ...
        data = expect_response(r, label="session-stats shape").status_ok().data
...
def test_retention_403_for_non_super(owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]) -> None:
...
def test_retention_returns_cohort_grid(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем retention за 4 недели"):
        ...
        data = expect_response(r, label="retention cohort").status_ok().data
...
def test_retention_clamps_weeks_to_max_26(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
...
def test_time_to_aha_403_for_non_super(owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]) -> None:
...
def test_time_to_aha_returns_percentiles_and_buckets(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем time-to-aha за 90 дней"):
        ...
        data = expect_response(r, label="time-to-aha shape").status_ok().data
...
def test_funnel_detail_returns_step_metrics(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем funnel-detail за 30 дней"):
        ...
        data = expect_response(r, label="funnel-detail").status_ok().data
...
def test_alerts_403_for_non_super(owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]) -> None:
...
def test_alerts_returns_items_list(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем алерты"):
        ...
        data = expect_response(r, label="alerts list").status_ok().data
...
def test_alerts_each_item_has_severity_title_message(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    """TC-PA-ANALYTICS-6.3: контракт каждого элемента."""
    with step("действие: запрашиваем алерты"):
        r = tenant_client(superadmin_user).get(routes.PLATFORM_ALERTS)
        items = expect_response(r, label="alerts items").status_ok().data["items"]
...
def test_health_403_for_non_super(owner_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]) -> None:
...
def test_health_returns_canonical_metrics(
    superadmin_user: AuthUser, tenant_client: Callable[[AuthUser], httpx.Client]
) -> None:
    ...
    with step("действие: запрашиваем health-метрики"):
        ...
        data = expect_response(r, label="health metrics").status_ok().data

    with step("проверка: канонические поля и free_cap_fill_ratio в диапазоне"):
        for key in (
            "events_last_hour",
            "usage_cents_last_day",
            "active_users",
            "free_cap",
            "free_cap_fill_ratio",
        ):
